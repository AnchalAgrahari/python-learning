...
#adding tag
def add_tag_from_local_file(bucket_name, data_local):
    try:
        client = boto3.client('s3')     
        tag_set = [{'Key': key, 'Value': value} for key, value in data_local.items()]
...
if __name__ == '__main__':
    try:
        ...
        data_local = read_file_from_local(file_name)
        print("Let's check data", data_local)
       
        add_tag_from_local_file(bucket_name, data_local)
